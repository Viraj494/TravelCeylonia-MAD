...
class EditBookingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val nameEdit: TextView = findViewById(R.id.vehicleNameEdit)
        val dateFromEdit: TextView = findViewById(R.id.editTextDateFromEdit)
        val dateToEdit: TextView = findViewById(R.id.editTextDateToEdit)
        val costEdit: TextView = findViewById(R.id.costTxtEdit)
        var update : Button = findViewById<Button>(R.id.updateBookingButton)
        var calc : Button = findViewById<Button>(R.id.calculateCostButtonEdit)
        ...
        val booking = intent.getParcelableExtra<BookingDataManage>("booking")
        if(booking != null){

            nameEdit.text = booking.vehicle
            ...
            update.setOnClickListener{
                ...
                if(dateFromEdit.text.toString().isEmpty() || dateToEdit.text.toString().isEmpty()){
                    ...
                    val cost = booking.cost
                    val totalCost = cost * dateDifference

                    val user = Firebase.auth.currentUser
                    if (user != null) {
                        var bookings = BookingData(booking.vehicle,dateFromEdit.text.toString() , dateToEdit.text.toString(),dateDifference,totalCost,user.email.toString(), cost,"no")
            ...
            calc.setOnClickListener{
                if(dateFromEdit.text.toString().isEmpty() || dateToEdit.text.toString().isEmpty()){
                    ...
                    val dateDifference = getDateDiff(SimpleDateFormat("dd/MM/yyyy"), dateFromEdit.text.toString(), dateToEdit.text.toString()) + 1
                    val totalCost = booking.cost * dateDifference
